...
try:
    ...
    cuda_capability = "0.0"  # 默认值

if cuda_capability not in shm_size_dict:
    # 使用备用实现
    try:
        from .flash_deform_attn_interface import flash_deform_attn_cpu as flash_deform_attn_cuda
    except ImportError:
        # 如果CPU实现也不存在，创建一个空实现
        class FlashDeformAttnFunction:
            ...
            def forward(ctx, *args, **kwargs):
                # 返回一个空张量或抛出更友好的错误
                print("警告: DCNv4在CPU模式下使用空实现")
                return args[0] if args else None
        flash_deform_attn_cuda = FlashDeformAttnFunction
