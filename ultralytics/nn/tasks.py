...
from .modules.AddModules import *   ###liuya###
from ultralytics.nn.modules.FocalModulation import *    ###liuya### 有参数的注意力机制修改

import torch
import torch.nn as nn

###liuya### add AKConv, SPDConv
...
###liuya###
from ultralytics.nn.modules.ContextGuide import ContextGuidedBlock_Down, C2f_Context
...
class MultiModel(MultiBaseModel):
    """YOLOv8检测和分割模型，YOLOv8 detection and segmentation model."""
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, C2f_Context, AKConv, SPDConv, CSPHet, C2f_DLKA, C2f_DCNv4, RepNCSPELAN4, RepNCSPELAN4_high):    ###liuya### add C2f_Context, AKConv, SPDConv, CSPHet, C2f_DLKA, C2f_DCNv4
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)

            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, C2f_Context, AKConv, SPDConv, CSPHet, C2f_DLKA, C2f_DCNv4, RepNCSPELAN4, RepNCSPELAN4_high):   ###liuya### add C2f_Context, AKConv, SPDConv, CSPHet, C2f_DLKA, C2f_DCNv4
                args.insert(2, n)  # number of repeats
                n = 1
        ###liuya### add elif方法
        elif m in {ContextGuidedBlock_Down, deformable_LKA_Attention}:
            c2 = ch[f]
            args = [c2, *args]
        ###########
        ###liuya### add elif ACmix方法
        elif m in {ACmix}:
            args = [ch[f], ch[f]]
        ###########
        elif m is AIFI:
            ...
            args = [ch[f]]
        elif m in {FocalModulation}:    ###liuya### 有参数的注意力机制修改
            args = [ch[f], *args]
        elif m is Concat:
...
def yaml_model_load(path):
    """Load a YOLOv8 model from a YAML file.从一个YAML文件中加载YOLOv8模型"""
    import re                                   # Python的正则表达式模块，用于处理字符串匹配和替换

    path = Path(path)                           # 将输入的路径转换成Path对象，通常用于文件系统操作
    if path.stem in (f'yolov{d}{x}6' for x in 'nsmlx' for d in (5, 8)):         # .stem获取路径的主干部分，判断是否是yolov5n6...
        new_stem = re.sub(r'(\d+)([nslmx])6(.+)?$', r'\1\2-p6\3', path.stem)    # 替换正则表达式的内容
        LOGGER.warning(f'WARNING ⚠️ Ultralytics YOLO P6 models now use -p6 suffix. Renaming {path.stem} to {new_stem}.')
        path = path.with_stem(new_stem)         # 创建一个新的Path对象，其主干部分（stem）被替换为new_stem

    unified_path = re.sub(r'(\d+)([nslmx])(.+)?$', r'\1\3', str(path))      # 统一路径格式：i.e. yolov8x.yaml -> yolov8.yaml
    yaml_file = check_yaml(unified_path, hard=False) or check_yaml(path)    # 检查经过处理后的文件路径或与原始文件路径是否存在对应的YAML文件
    d = yaml_load(yaml_file)                    # 加载YAML文件并将其内容解析为一个字典，存储在变量d中
    if not d.get('scale', False):               # 如果字典d中不包含键'scale'，则通过调用guess_model_scale(path)函数来猜测模型的尺度，并将结果存储在键'scale'中
        d['scale'] = guess_model_scale(path)
    d['yaml_file'] = str(path)                  # 将加载的YAML文件路径转换成字符串，并存储在字典d的'yaml_file'键中
    return d                                    # 最终，返回包含加载的模块信息、尺度信息和文件路径的字典d


def guess_model_scale(model_path):
    """
    根据给定的YOLO模型的YAML文件路径提取尺度的大小字符，并返回该字符。
    - `n`：通常代表“小”（small），指的是较小尺寸的模型，更适合于在资源受限的环境下运行，速度可能会更快。
    - `s`：代表“中等”（medium），一般比小尺寸的模型稍大，平衡了精度和速度。
    - `m`：代表“中等”或“中等以上”（medium or medium-large），通常介于中等到大型尺寸之间，可提供更好的精度。
    - `l`：代表“大”（large），指的是相对较大的模型，通常具有更高的精度但速度较慢，适用于需要更高准确性的任务。
    - `x`：通常代表“极大”（extra large）或“特大”（extraordinary），是最大尺寸的模型，提供最高的精度但速度最慢，适用于要求非常高准确性的任务。
...
def guess_model_task(model):
    """
    用于猜测PyTorch模型的任务（detect/segment/classify/pose）
