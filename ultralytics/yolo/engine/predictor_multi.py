...
class BasePredictor:
    ...
    def save_preds(self, vid_cap, idx, save_path):
        ...
        if self.dataset.mode == 'image':
            ...
            # 确保掩码的形状与图像的形状一致
            color_mask1 = cv2.resize(color_mask1, (im0.shape[1], im0.shape[0]))
            color_mask2 = cv2.resize(color_mask2, (im0.shape[1], im0.shape[0]))

            alpha = 0.5  # transparency factor

            # Overlay masks on im0 with transparency
            im0[color_mask1 != [0, 0, 0]] = (1 - alpha) * im0[
                color_mask1 != [0, 0, 0]] + alpha * color_mask1[
                                                     color_mask1 != [0, 0, 0]]
            im0[color_mask2 != [0, 0, 0]] = (1 - alpha) * im0[
                color_mask2 != [0, 0, 0]] + alpha * color_mask2[
                                                     color_mask2 != [0, 0, 0]]
