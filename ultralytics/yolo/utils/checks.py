...
def check_pip_update_available():
    ...
    检查是否有最新版的Ultralytics可用包，提示用户更新，以获取新功能、改进和修复的内容。
    Returns:
...
def check_file(file, suffix='', download=True, hard=True):
    ...
    check_suffix(file, suffix)  # optional检查文件的后缀名是否符合指定的后缀
    file = str(file).strip()    # convert to string and strip spaces将文件路径转换成字符串并去除空格
    file = check_yolov5u_filename(file)  # yolov5n -> yolov5nu对文件名进行一些特定操作
    if not file or ('://' not in file and Path(file).exists()):  # exists ('://' check required in Windows Python<3.10)如果文件已存在，则直接返回文件路径
        return file
    elif download and file.lower().startswith(('https://', 'http://', 'rtsp://', 'rtmp://')):  # download如果需要下载文件，并且文件路径是以特定协议开头（https://,http://），则下载文件到本地
        ...
        return file
    else:  # search否则，在指定的目录中搜索文件
        files = []
        for d in 'models', 'datasets', 'tracker/cfg', 'yolo/cfg':  # search directories
            files.extend(glob.glob(str(ROOT / d / '**' / file), recursive=True))  # find file
        if not files and hard:
            raise FileNotFoundError(f"'{file}' does not exist")
        elif len(files) > 1 and hard:               # 如果找到多个匹配项且要求精确匹配（hard=True），则引发异常。
            raise FileNotFoundError(f"Multiple files match '{file}', specify exact path: {files}")
        return files[0] if len(files) else []       # return file，返回找到的文件路径


def check_yaml(file, suffix=('.yaml', '.yml'), hard=True):
    """
    Search/download YAML file (if necessary) and return path, checking suffix.
    根据提供的文件路径(file)以及后缀名列表（默认.yaml和.yml），调用check_file函数来进行文件检查，返回check_file函数的返回值。
    """
