...
def yaml_load(file='data.yaml', append_filename=False):
    ...
    Args:
        file (str, optional): 指定要加载数据的YAML文件，默认为data.yaml。File name. Default is 'data.yaml'.
        append_filename (bool): 控制是否将YAML文件名添加到返回的字典中，默认为False。Add the YAML filename to the YAML dictionary. Default is False.

    Returns:
        dict: YAML data and file name.
    """
    with open(file, errors='ignore', encoding='utf-8') as f:    # 打开指定的YAML文件，以UTF-8编码读取文件内容，把此文件对象赋值给f变量
        s = f.read()                                            # 读取文件全部内容，返回一个字符串。string

        # Remove special characters，如果文件内容中存在非可打印字符，则通过正则表达式取出这些特殊字符。
        if not s.isprintable():
            s = re.sub(r'[^\x09\x0A\x0D\x20-\x7E\x85\xA0-\uD7FF\uE000-\uFFFD\U00010000-\U0010ffff]+', '', s)

        # 解析刘中的第一个YAML文档，并生成相应的Python对象
        # Add YAML filename to dict and return，根据append_filename的值，选择是否将加载的YAML数据与文件名合并成一个字典返回，或者只返回加载的YAML数据。
