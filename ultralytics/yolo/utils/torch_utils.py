...
def select_device(device='', batch=0, newline=False, verbose=True):
    """选择PyTorch设备：Selects PyTorch Device. Options are device = None or 'cpu' or 0 or '0' or '0,1,2,3'."""
    s = f'Ultralytics YOLOv{__version__} 🚀 Python-{platform.python_version()} torch-{torch.__version__} '   # 字符串s包含Ultralytics YOLOv的版本信息、Python的版本信息以及Torch的版本信息
    device = str(device).lower()                                        # 如果device=[0],返回的为字符串'[0]'，并转换成小写形式
    for remove in 'cuda:', 'none', '(', ')', '[', ']', "'", ' ':        # 循环遍历后面这些字符串，把device中的这些替换成‘’
        device = device.replace(remove, '')                             # to string, 'cuda:0' -> '0' and '(0, 1)' -> '0,1'and '[0]' -> '0'
    cpu = device == 'cpu'                                               # 判断device是否等于字符串'cpu'，如果相等，则cpu赋值为True
...
def init_seeds(seed=0, deterministic=False):
    """
    初始化随机数发生器（RNG）种子，以实现在注入random、numpy和torch等库中生成随机数的可重现性。
    当deterministic=True时，会应用额外的设置来强制训练过程具有确定性，例如设置确定性算法和CUBLAS工作区配置。
