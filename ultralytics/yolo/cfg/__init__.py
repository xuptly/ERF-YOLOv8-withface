...
def get_cfg(cfg: Union[str, Path, Dict, SimpleNamespace] = DEFAULT_CFG_DICT, overrides: Dict = None):
    """
    加载并合并来自文件或字典的配置数据：default.yaml和调用train的时候传的参数
    Load and merge configuration data from a file or dictionary.

    Args:
        cfg (str) or (Path) or (Dict) or (SimpleNamespace): Configuration data.配置数据
        overrides (str) or (Dict), optional: Overrides in the form of a file name or a dictionary. Default is None.覆盖项

    Returns:
        (SimpleNamespace): Training arguments namespace.训练参数
    """
    cfg = cfg2dict(cfg)
