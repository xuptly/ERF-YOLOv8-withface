...
class DetectionSegmentationTrainer(BaseTrainer):
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        """
        初始化一个DetectionSegmentationTrainer对象
        Initialize a DetectionSegmentationTrainer object with given arguments.
        """
        if overrides is None:
            overrides = {}
        overrides['task'] = 'multi'
        super().__init__(cfg, overrides, _callbacks)    # 调用父类初始化方法，进行参数初始化、目录和路径的设置、设备的设置、模型和数据集的设置、优化工具相关的初始化设置、每个周期（epoch）级别的指标的初始化设置和回调函数的设置
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        """用于返回一个YOLO多模型（MultiModel）对象，Return a YOLO multi model."""
    ...
    def progress_string(self):
        """
        用于生成格式化的训练进度信息字符串，包括当前的epoch、GPU内存占用、损失值、实例数和数据大小等。
    ...
    def plot_training_samples(self, batch, ni, task=None):
        ######Jiayuan
        """
        用于绘制带有注释的训练样本图像，生成的图像文件名根据任务类型和批次号命名，保存在指定的保存目录下。
...
def train(cfg=DEFAULT_CFG, use_python=False):
    """用于训练和优化YOLO模型，根据提供的训练数据和设备。Train and optimize YOLO model given training data and device."""
    ...
    if use_python:
        ...
        trainer.train()         # 调用DetectionSegmentationTrainer的train方法，开始训练
